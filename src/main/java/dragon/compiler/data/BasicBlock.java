...
public class BasicBlock {
	...
	public void createPhiFunction(int ident, BasicBlock curBlock) throws SyntaxFormatException {
		Instruction instr = phiFuncManager.createPhiInstruction(ident,
				VariableManager.getLastVersionSSA(ident));
//		if(curBlock.kind == Type.DO){
//			updateVarReferenceToPhi(ident, instr.getSelfPC(), curBlock);
//		}
	}
	...
	public void updateVarReferenceToPhi(int ident, int newSSA, BasicBlock curBlock) throws SyntaxFormatException{
		...
		// update loop body
		if(curBlock.kind != Type.DO)
			throw new SyntaxFormatException("updateVarReferenceToPhi can only be called in the LOOP BODY!");
